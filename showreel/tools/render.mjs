...
// ---------- static server
const TYPES = { '.html': 'text/html', '.js': 'text/javascript', '.css': 'text/css', '.ttf': 'font/ttf', '.jpg': 'image/jpeg', '.json': 'application/json', '.png': 'image/png' };
...
// ---------- post: lateral chromatic aberration, vignette, grain, fade
...
function finish(acc, n, fx, frameNo) {
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      let r = acc[(y * W + xr) * 3] * inv;
      let g = acc[i * 3 + 1] * inv;
      let b = acc[(y * W + xb) * 3 + 2] * inv;
      ...
      r = (r * v + gr) * fx.fade; g = (g * v + gr) * fx.fade; b = (b * v + gr) * fx.fade;
      const o = i * 3;
...
async function renderFrame(page, f, sub) {
  if (sub === 'auto') sub = SUB_OVERRIDE || (await page.subframes(f / FPS));
...
if (args.stills || args.sheet) {
  ...
  if (args.sheet) for (let t = 0; t < DURATION; t += +args.sheet) times.push(+t.toFixed(3));
  ...
  const ff = spawn('ffmpeg', ['-y', '-hide_banner', '-loglevel', 'error',
    ...
    '-c:v', 'libx264', '-preset', 'slow', '-crf', String(args.crf || 18), '-x264-params', 'aq-mode=3:psy-rd=1.0,0.0',
  ...
  await Promise.all(pages.map(async (pg) => {
    while (next < f1) {
      ...
      done.set(f, await renderFrame(pg, f, SUB));
      await flush();
    }
  }));
  await flush();
