...
//   options: --workers 3  --sub 1 (force motion-blur subframes)  --from 0 --to 15  --crf 19  --blur (stills with blur)
...
if (args.stills || args.sheet) {
  ...
  const ff = spawn('ffmpeg', ['-y', '-hide_banner', '-loglevel', 'error',
    ...
    '-c:v', 'libx264', '-preset', args.fast ? 'veryfast' : 'slow', '-crf', String(args.crf || 19), '-x264-params', 'aq-mode=3:psy-rd=1.0,0.0',
