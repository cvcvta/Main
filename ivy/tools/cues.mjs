...
import { typeSchedule, navHops, adminAt, taglineAt } from '../src/timing.js';
...
console.log(JSON.stringify({
  ...
  admin: COPY.admin.split(' ').map((_, i) => adminAt(i)),
  typing: [...COPY.prompt].map((ch, i) => ({ t: typeAt[i], ch })),
  nav: navHops(),
  tagline: COPY.oneLiner.split(' ').map((_, i) => taglineAt(i)),
}, null, 1));
