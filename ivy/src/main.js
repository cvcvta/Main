...
// the spot opens out of black
const black = el('div', 'abs', stage);
css(black, { left: '0px', top: '0px', width: '1920px', height: '1080px', background: '#000' });
...
const SPLIT = 0.85; // the halves fall away, seconds

function seek(t) {
  // night scene, split in two along the light
  const splitP = seg(t, T.split, SPLIT, E.lin);
  const nightOn = t < T.split + SPLIT + 0.04;
  ...
  const lineOn = t >= T.slice && t < T.split + 0.5;
  const len = (s1 - s0) * lp;
  const fade = 1 - seg(t, T.split + 0.05, 0.4, E.outQuad);
  ...
  const fl = seg(t, T.split, 0.04) * (1 - seg(t, T.split + 0.04, 0.55, E.outQuad));
  css(flash, {
  ...
  // "One platform."
  const supIn = (i) => seg(t, T.platform + 0.1 + i * S16 * 1.3, 0.6, E.snap);
  const supOut = seg(t, T.pullOut + 0.04, 0.4, E.inCubic);
  css(sup, { display: t > T.platform + 0.05 && t < T.pullOut + 0.5 ? 'block' : 'none', left: '1200px', top: '330px', opacity: String(1 - supOut), transform: `translate3d(${(-120 * supOut).toFixed(2)}px, ${(-40 * supOut).toFixed(2)}px, 0) scale(${lerp(1, 0.9, supOut).toFixed(4)})` });
  // 150 px clears the mask's descender padding, so nothing peeks out before a word rises
  supLines.forEach((s, i) => css(s, { transform: `translate3d(0, ${(150 * (1 - supIn(i))).toFixed(2)}px, 0)` }));
  ...
  const bk = 1 - seg(t, 0, 0.45, E.outQuad);
  css(black, { display: bk > 0 ? 'block' : 'none', opacity: bk.toFixed(4) });
}

// Per-frame post settings for tools/render.mjs.
function post(t) {
  // the night's heavy vignette and grain lift as the halves fall away; the end card sits in between
  const night = 1 - seg(t, T.split, SPLIT, E.inOutQuad);
  const endc = seg(t, T.zoom + 0.1, 0.5, E.inOutQuad);
  const hit = (t0, a, k) => (t >= t0 ? a * Math.exp(-(t - t0) * k) : 0);
  return {
    vignette: lerp(lerp(0.34, 0.5, endc), 0.62, night),
    grain: lerp(lerp(0.03, 0.035, endc), 0.05, night),
    ca: hit(T.slam, 3.2, 7) + hit(T.split, 4.5, 6) + hit(T.tap, 1.6, 9) + hit(T.zoom + 0.38, 2.2, 6),
  };
}
...
function subframes(t) {
  ...
  if (within(T.slam - 0.08, T.slam + 0.3) || within(T.split - 0.2, T.split + SPLIT + 0.04) || within(T.zoom, T.zoom + 0.85)) return 16;
  if (within(T.fly - 0.05, T.land + 0.15) || within(T.morph, T.lockup + 0.1) || within(T.pullOut, T.pullOut + 1.0) || within(T.intro, T.intro + 0.75)) return 14;
  if (within(T.crane, T.pullOut) || within(T.pushIn, T.pushIn + 0.9) || within(T.send, T.plan + 0.5) || within(T.tap - 0.1, T.tap + 0.4) || within(T.slice, T.split)) return 10;
